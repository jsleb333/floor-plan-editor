"""Startup task that installs the bundled demo plan on first run."""
...
class DemoPlanSeeder:
    ...
    Role:
        ...
        the very first time it starts. It is an application startup task, not a
        core orchestrator: seeding this instance's store is a bootstrap concern,
        so it lives in the composition root and is invoked only from the
        application entry point (the ``main`` lifespan). Seeding is best-effort:
