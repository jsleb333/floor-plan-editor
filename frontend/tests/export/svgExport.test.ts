...
describe('buildPlanSvg', () => {
  ...
  it('fills and outlines each wall in its own colour, and its openings with it (spec S1f)', () => {
    ...
    expect(svg).toContain(
      `fill="${EXTERIOR_WALL_COLOR}" fill-rule="evenodd" stroke="${EXTERIOR_WALL_COLOR}"`,
    )
    expect(svg).toContain(
      `fill="${INTERIOR_WALL_COLOR}" fill-rule="evenodd" stroke="${INTERIOR_WALL_COLOR}"`,
    )
  ...
  it('honours a wall colour override over the role default (spec S1f)', () => {
    ...
    expect(svg).toContain('fill="#b91c1c" fill-rule="evenodd" stroke="#b91c1c"')
