 ...
 * Fill and outline both take the wall's own colour — override or role default,
 * the same `wallColor` rule the canvas applies (spec S1f). A bevel wedge takes
 * the colour of the first wall its joint names, like the canvas.
 */
function renderWalls(
  ...
  for (const wall of walls) {
    ...
    out.push(`<path d="${fill}" fill="${color}" fill-rule="evenodd" stroke="none" />`)
    ...
    if (stroke !== '') {
      out.push(
        `<path d="${stroke}" fill="none" stroke="${color}" stroke-width="${STRUCTURE_STROKE_IN}" />`,
      )
    }
  }
  const jointWall = new Map(joints.map((joint) => [joint.id, wallIdsOf(joint)[0]] as const))
  for (const gap of network.gaps) {
    const wall = byId.get(jointWall.get(gap.jointId) ?? '')
    const color = wall ? wallColor(wall, presetsIn) : INTERIOR_WALL_COLOR
    out.push(`<path d="${ringsToPath([[...gap.points]])}" fill="${color}" stroke="none" />`)
  }
  return out
}
...
export function buildPlanSvg(document: PlanDocument, options: SvgExportOptions = {}): string {
  ...
  const structure = [
    ...
    layer(
      'walls',
      renderWalls(document.walls, wallNetworkOf(document), document.joints, presetsIn),
    ),
    layer(
      'openings',
