...
class TestPlanService:
    ...
    async def test_create_plan__when_called__persists_fresh_plan_with_defaults(
        ...
        """A new plan starts at revision 1 with an empty schema v6 document, not archived."""
        plan = await service.create_plan("Basement")

        assert plan.name == "Basement"
        assert plan.revision == 1
        assert plan.archived_at is None
        assert plan.document.schema_version == 6
    ...
    async def test_update_document__when_body_claims_an_old_schema_version__stores_current_version(
        ...
        """A v2-shaped body validated against the current model is persisted claiming schema v6, not the stale v2 it was sent with."""
        repo.update_document.return_value = 5
...
class TestPlanServiceMigration:
    ...
    async def test_get_plan__when_stored_document_is_v1__returns_migrated_v6_and_keeps_backup(
        ...
        assert stored.document["schema_version"] == 6
