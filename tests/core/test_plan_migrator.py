...
class TestPlanMigrator:
    ...
    def test_migrate__when_document_is_v1__walks_all_steps_to_v5(
        self, migrator: PlanMigrator
    ) -> None:
        """A v1 document gains the empty structure collections, the default thickness presets, the empty underlay slot, the empty device collections and the empty electrical-layout collections, while keeping its viewport; the result validates as a PlanDocument."""
        raw = _v1_document()
    ...
    def test_migrate__when_document_is_v2__adds_underlay_device_and_electrical_collections(
        ...
        """A v2 document gains the underlay slot, the device and electrical-layout collections and schema version 5, keeping its structure content; the result validates as a PlanDocument."""
        raw = _v2_document()
    ...
    def test_migrate__when_document_is_v4__adds_empty_electrical_collections(
        self, migrator: PlanMigrator
    ) -> None:
        """A v4 document only gains the circuits, wires and control-link collections and schema version 5; the result validates as a PlanDocument."""
        raw = _v4_document()
